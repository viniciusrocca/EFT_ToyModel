...
def getInfo(f,nlo = False,labelsDict=None):
    ...
    if labelsDict is None:
        labelsDict = {
                      'DMsimp_s_spin1': 'Z prime', 'z_prime': 'Z prime',
...
def get_params_from_filename(run_dir):
    ...
    mZp = parts[idx - 1]
    return f"mZp_{mZp}", float(mZp)

def AddInfoToDistributions(distributions, params, info, nlo = False, bias = False):
    ...
    distributions['mZp'] = info['mZp']
    distributions['gvd11'] = info['gvd11']
    distributions['gvu11'] = info['gvu11']
    distributions['gvd22'] = info['gvd22']
    distributions['gvu22'] = info['gvu22']
    distributions['gvd33'] = info['gvd33']
    distributions['gvu33'] = info['gvu33']
...
def main():
    ...
    with tqdm(total=len(run_dirs), unit="run", dynamic_ncols=True) as pbar:
        for i, run_dir in enumerate(run_dirs):
            ...
            identifier_string, mZp = get_params_from_filename(run_dir)
            ...
            try:
                ...
                if distributions and 'mTT' in distributions and distributions['mTT'].size > 0:
                    distributions = AddInfoToDistributions(distributions, params, info, nlo, is_biased_run)

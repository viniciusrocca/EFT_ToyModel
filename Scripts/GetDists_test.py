...
def getDistributions(filename):
    """
    RAM-OPTIMIZED: Uses an iterator to stream events one-by-one, 
    preventing massive memory spikes from object creation.
    """
    
    # 1. Read Header Info without loading events
    fixedFile = tempfile.mkstemp(suffix='.lhe')[1]
    try:
        ...
        nevents = pylhe.read_num_events(fixedFile)
        initBlock = pylhe.read_lhe_init(fixedFile)['procInfo'][0]
        xSec = initBlock['xSection']
        xSecErr = initBlock['error']
        
        # 2. Stream Events (Memory Safe)
        top_list = []
        atop_list = []
        w_list = []
        
        # pylhe.read_lhe_with_attributes is a generator. We iterate it directly 
        # instead of casting it to a list().
        for ev in pylhe.read_lhe_with_attributes(fixedFile):
            ...
            if p_t and p_at:
                ...
                w_list.append(w) # Only append weight if we keep the event!
                
            # FORCE GARBAGE COLLECTION OF THE HEAVY pylhe OBJECT
            del ev 
    ...
    P_t = np.array(top_list, dtype=np.float32) # Downcast to 32-bit to save 50% RAM
    P_at = np.array(atop_list, dtype=np.float32)
    weights = np.array(w_list, dtype=np.float64) / nevents

    # Free the lists from memory immediately before doing the heavy math
    del top_list, atop_list, w_list

    if len(P_t) == 0:
        return {}

    # 3. Vectorized Calculations (Unchanged)
...
def AddInfoToDistributions(distributions, params, mPsiT, mSDM, info, nlo = False, bias = False):
    ...
    converter_dict = {'TopEFT': 'VLF EFT', 'UV_BSM': '1-loop VLF', 'sm':'SM',
                      ...
                      'VLF': 'VLF', 'Scalar': 'Scalar', 'Scalar_ff': 'Scalar',
                      ...
                      'pp2ttbar_gs4_ydm4': r'$p p \to t \bar{t}$ ',
                      'pp2ttbar_gs6': r'$p p \to t \bar{t}$', 'qq2ttbar_gs4': r'$q q \to t \bar{t}$', 'gg2ttbar_gs4': r'$g g \to t \bar{t}$',
                      'pp2ttbar_gs4': r'$p p \to t \bar{t}$', 'gs4': r'$g_s^4$', 'gs6': r'$g_s^6$', 'ydm2': r'$y_{DM}^2$', 'ydm4': r'$y_{DM}^4$'}
...
def main():
    ...
    with tqdm(total=len(run_dirs), unit="run", dynamic_ncols=True) as pbar:
        for i, run_dir in enumerate(run_dirs):
            ...
            if params['model'] == 'UV_BSM' or params['model'] == 'SMS_1_loop' or params['model'] == 'VLF' or params['model'] == 'Scalar':
                try:

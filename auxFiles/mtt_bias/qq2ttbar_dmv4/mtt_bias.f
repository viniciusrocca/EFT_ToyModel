...
C --------------------
          ...
          if (mtt.gt.0.0d0) then
            if (mtt .lt. 1050.0d0) then
              ! Region 1 (Quadratic)
              polynomial_arg = - (-1.21464797d-06 * mtt**2 + 1.58937801d-03 * mtt - 3.12900951d+00)
              ! Offset calculated using Region 2 coefficients
              polynomial_offset =  - (7.74307606d-07 * mtt_bias_offset**2 - 1.82755301d-03 * mtt_bias_offset - 1.62630937d+00)

            else if (mtt .ge. 1050.0d0 .and. mtt .lt. 1650.0d0) then
              ! Region 2 (Quadratic)
              polynomial_arg = - (7.74307606d-07 * mtt**2 - 1.82755301d-03 * mtt - 1.62630937d+00)
              polynomial_offset =  - (7.74307606d-07 * mtt_bias_offset**2 - 1.82755301d-03 * mtt_bias_offset - 1.62630937d+00)

            else if (mtt .ge. 1650.0d0 .and. mtt .lt. 2250.0d0) then
              ! Region 3 (Cubic)
              polynomial_arg = - (-1.40293809d-09 * mtt**3 + 4.88451976d-06 * mtt**2 - 3.96609499d-03 * mtt - 3.03084589d+00)
              polynomial_offset =  - (7.74307606d-07 * mtt_bias_offset**2 - 1.82755301d-03 * mtt_bias_offset - 1.62630937d+00)

            else if (mtt .ge. 2250.0d0 .and. mtt .lt. 3100.0d0) then
              ! Region 4 (Quadratic)
              polynomial_arg = - (1.96643964d-07 * mtt**2 - 4.71209721d-03 * mtt + 6.47233027d+00)
              polynomial_offset =  - (7.74307606d-07 * mtt_bias_offset**2 - 1.82755301d-03 * mtt_bias_offset - 1.62630937d+00)

            else if (mtt .ge. 3100.0d0 .and. mtt .lt. 4500.0d0) then
              ! Region 5 (Quadratic)
              polynomial_arg = - (2.53134990d-07 * mtt**2 - 4.79843418d-03 * mtt + 6.24230757d+00)
              polynomial_offset =  - (7.74307606d-07 * mtt_bias_offset**2 - 1.82755301d-03 * mtt_bias_offset - 1.62630937d+00)

            else
            ! Region 6 (Quadratic)
            mtt = min(mtt, 6000.0d0)
              polynomial_arg = - (-2.53403371d-09 * mtt**2 - 2.54812138d-03 * mtt + 1.27284488d+00)
              polynomial_offset =  - (7.74307606d-07 * mtt_bias_offset**2 - 1.82755301d-03 * mtt_bias_offset - 1.62630937d+00)
          endif

...
c
      ...
      double precision, parameter :: bound_1 = 801.0d0
      double precision, parameter :: bound_2 = 1051.0d0
      double precision, parameter :: bound_3 = 1201.0d0
      double precision, parameter :: bound_4 = 2101.0d0
      double precision, parameter :: mtt_norm = 800.0d0
...
c        Calculate the normalization offset using the first polynomial at mtt_norm
         p_offset = -(-2.9610d-06*mtt_norm**2 - 4.7735d-03*mtt_norm - 1.6135d+00)
         
c        Pre-calculate the function values at each boundary for stitching
         p1_at_b1 = -(-2.9610d-06*bound_1**2 - 4.7735d-03*bound_1 - 1.6135d+00)
         p2_at_b1 = -( 1.4464d-04*bound_1**2 - 2.7053d-01*bound_1 + 1.1679d+02)
         
         p2_at_b2 = -( 1.4464d-04*bound_2**2 - 2.7053d-01*bound_2 + 1.1679d+02)
         p3_at_b2 = -( 6.5156d-04*bound_2**2 - 1.4763d+00*bound_2 + 8.2438d+02)

         p3_at_b3 = -( 6.5156d-04*bound_3**2 - 1.4763d+00*bound_3 + 8.2438d+02)
         p4_at_b3 = -(-5.9226d-07*bound_3**2 - 1.0188d-03*bound_3 - 7.9457d+00)

         p4_at_b4 = -(-5.9226d-07*bound_4**2 - 1.0188d-03*bound_4 - 7.9457d+00)
         p5_at_b4 = -( 1.7820d-07*bound_4**2 - 3.5380d-03*bound_4 - 5.7652d+00)

c --- Main piecewise function with stitching ---
         if (mtt .lt. bound_1) then
c           --- REGION 1 ---
            p_val = -(-2.9610d-06*mtt**2 - 4.7735d-03*mtt - 1.6135d+00)
            bias_wgt = EXP(p_val - p_offset)

         else if (mtt .lt. bound_2) then
c           --- REGION 2 (Stitched) ---
            p_val = -( 1.4464d-04*mtt**2 - 2.7053d-01*mtt + 1.1679d+02)
            bias_wgt = EXP(p_val + (p1_at_b1 - p2_at_b1) - p_offset)
            
         else if (mtt .lt. bound_3) then
c           --- REGION 3 (Stitched) ---
            p_val = -( 6.5156d-04*mtt**2 - 1.4763d+00*mtt + 8.2438d+02)
            bias_wgt = EXP(p_val + (p1_at_b1 - p2_at_b1) + (p2_at_b2 - p3_at_b2) - p_offset)

         else if (mtt .lt. bound_4) then
c           --- REGION 4 (Stitched) ---
            p_val = -(-5.9226d-07*mtt**2 - 1.0188d-03*mtt - 7.9457d+00)
            bias_wgt = EXP(p_val + (p1_at_b1 - p2_at_b1) + (p2_at_b2 - p3_at_b2) + (p3_at_b3 - p4_at_b3) - p_offset)
            
         else
c           --- REGION 5 (Stitched) ---
            mtt = min(mtt,3500.0)
            p_val = -( 1.7820d-07*mtt**2 - 3.5380d-03*mtt - 5.7652d+00)

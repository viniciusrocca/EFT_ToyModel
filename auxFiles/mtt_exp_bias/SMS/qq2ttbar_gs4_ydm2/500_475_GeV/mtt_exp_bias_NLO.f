...
c --- Boundary and Normalization Parameters ---
      ...
      double precision, parameter :: bound_3 = 1800.0d0
      double precision, parameter :: bound_4 = 2100.0d0
      double precision, parameter :: bound_5 = 2450.0d0
      double precision, parameter :: bound_6 = 2700.0d0
      double precision, parameter :: mtt_norm = 500.0d0
...
c        Calculate the normalization offset using the first polynomial
         p_offset = -(- 1.9946d-05*mtt_norm**2 + 2.1331d-02*mtt_norm - 1.2603d+01)
         
c        Pre-calculate the function values at each boundary for stitching
         p1_at_b1 = -(- 1.9946d-05*bound_1**2 + 2.1331d-02*bound_1 - 1.2603d+01)
         p2_at_b1 = -( 2.7444d-06*bound_1**2 - 6.4787d-03*bound_1 - 3.9998d+00)
         
         p2_at_b2 = -( 2.7444d-06*bound_2**2 - 6.4787d-03*bound_2 - 3.9998d+00)
         p3_at_b2 = -( 2.5239d-07*bound_2**2 - 5.0366d-03*bound_2 - 2.4983d+00)

         p3_at_b3 = -( 2.5239d-07*bound_3**2 - 5.0366d-03*bound_3 - 2.4983d+00)
         p4_at_b3 = -( 1.2310d-05*bound_3**2 - 5.1804d-02*bound_3 + 4.2606d+01)

         p4_at_b4 = -( 1.2310d-05*bound_4**2 - 5.1804d-02*bound_4 + 4.2606d+01)
         p5_at_b4 = -(- 2.5509d-05*bound_4**2 + 1.0822d-01*bound_4 - 1.2731d+02)

         p5_at_b5 = -(- 2.5509d-05*bound_5**2 + 1.0822d-01*bound_5 - 1.2731d+02)
         p6_at_b5 = -( 1.1844d-04*bound_5**2 - 6.1110d-01*bound_5 + 7.7113d+02)
         
         p6_at_b6 = -( 1.1844d-04*bound_6**2 - 6.1110d-01*bound_6 + 7.7113d+02)
         p7_at_b6 = -(- 1.5731d-06*bound_6**2 + 9.1852d-03*bound_6 - 2.8470d+01)


         if (mtt .lt. bound_1) then
c           --- REGION 1 ---
            p_val = -(- 1.9946d-05*mtt**2 + 2.1331d-02*mtt - 1.2603d+01)
            bias_wgt = EXP(p_val - p_offset)

         else if (mtt .lt. bound_2) then
c           --- REGION 2 (Stitched) ---
            p_val = -( 2.7444d-06*mtt**2 - 6.4787d-03*mtt - 3.9998d+00)
            bias_wgt = EXP(p_val + (p1_at_b1 - p2_at_b1) - p_offset)
            
         else if (mtt .lt. bound_3) then
c           --- REGION 3 (Stitched) ---
            p_val = -( 2.5239d-07*mtt**2 - 5.0366d-03*mtt - 2.4983d+00)
            bias_wgt = EXP(p_val + (p1_at_b1 - p2_at_b1) + (p2_at_b2 - p3_at_b2) - p_offset)

         else if (mtt .lt. bound_4) then
c           --- REGION 4 (Stitched) ---
            p_val = -( 1.2310d-05*mtt**2 - 5.1804d-02*mtt + 4.2606d+01)
            bias_wgt = EXP(p_val + (p1_at_b1 - p2_at_b1) + (p2_at_b2 - p3_at_b2) + (p3_at_b3 - p4_at_b3) - p_offset)
            
         else if (mtt .lt. bound_5) then
c           --- REGION 5 (Stitched) ---
            p_val = -(- 2.5509d-05*mtt**2 + 1.0822d-01*mtt - 1.2731d+02)

...
\begin{document}
		...
		\begin{titlepage}		
			...
			\begin{center}
				...
				{\large {\bf Student}: Vinícius Zilio Rocca\\
					\vspace{40pt}
					\underline{\hspace{8cm}}\\
					{\bf Supervisor}: André Paniago Lessa}
			\end{center}
			
			\vspace{80pt}	
			\begin{flushleft}
				{\large \bf Master dissertation}
			\end{flushleft}
			
			\vspace{90pt}
			\begin{center}
				{\large \it Santo André\\
					\today}
			\end{center}
	...
	After the discovery of the Higgs boson the Standard Model has been established as the correct description of physics at energies around and below the electroweak scale. With the further increase of LHC data and no clear evidence of new physics, arises the possibility that new physics might be beyond the LHC energy reach. Nonetheless, indirect BSM effects on SM observables can still be probed using effective field theory (EFT) framework. This work first presents a detailed investigation of the Covariant Derivative Expansion (CDE) method for systematically integrating out heavy fields, with a pedagogical application to a toy model. It then focuses on a phenomenological analysis of observables linked to top-antitop ($t\bar{t}$) production at the LHC, using an EFT derived from a BSM model with a vector-like top partner and a scalar dark matter candidate. By comparing the EFT prediction for a specific interference contribution to the total cross section with the full one-loop ultraviolet (UV) result, the study provides a preliminary assessment of the EFT’s prediction both within and beyond its regime of validity for this observable. The results show that the EFT accurately reproduces the UV prediction up to BSM masses around 4 TeV, underestimates it by roughly 10\% in the 1-4 TeV range, and rapidly loses accuracy below 800 GeV.
	...
	\end{align}
	which governs the dynamics of the strong, weak, and electromagnetic interactions.
	...
	\begin{enumerate}
		...
		\begin{align}
			SU(2)_L \times U(1)_Y \longrightarrow U(1)_{Q}, \nonumber
		\end{align}
		which gives mass to the $W^\pm$ and $Z$ bosons while preserving the unbroken $U(1)_{Q}$ of quantum electrodynamics (QED).
	\end{enumerate}
	...
	\end{align}
	where $T^A = \lambda^A/2$ and $t^I = \tau^I/2$ are the $SU(3)$ and $SU(2)$ generators, respectively, with $\lambda^A$ and $\tau^I$ denoting the Gell-Mann and Pauli matrices. The couplings $g_s$, $g_w$, and $g_1$ denote the couplings of the strong, weak, and hypercharge interactions, respectively.
	
	The third line of the Lagrangian includes the kinetic term for the Higgs field, $(D_\mu H)^\dagger (D^\mu H)$, and the Higgs scalar potential, $V(H) = - m^2 H^\dagger H + \frac{\lambda}{2} (H^\dagger H)^2$. Minimizing this potential leads to a non-vanishing vacuum expectation value (VEV) for the Higgs field, $v^2 = 2 \bra{0} H^\dagger H \ket{0}$, whose tree-level expression is $v^2 = (2 m^2)/\lambda$. This phenomenon is known as spontaneous electroweak symmetry breaking, $SU(2)_L \times U(1)_Y \longrightarrow U(1)_{Q}$, which is crucial for imparting mass to the $W^\pm$ and $Z$ bosons as well as all fundamental fermions. For convenience, the Higgs doublet can be reparameterized as:	
	\begin{align}
		H = \frac{1}{\sqrt{2}}
		\begin{pmatrix}
			\varphi_2 + i \varphi_1 \\
	...
	While Fermi's theory is an excellent low-energy approximation, it is not a complete theory. It violates unitarity at energies near $M_W$ and is non-renormalizable. This is a characteristic of EFTs: within it's domain of validity, it provides simpler ways to compute accurately theoretical predictions than in the full model, but they are destined to break down as the energy scale becomes non-negligible compared to that of the new physics. 
	...
	\begin{align}
		\mathcal{L}_{\text{EFT}} = \mathcal{L}_{d \le 4} + \sum_{d=5}^{\infty} \frac{1}{M^{d-4}} \sum_{i=1}^{n_d} C_{i}^{(d)} O_{i}^{(d)}
	\end{align}
	where $\mathcal{L}_{d \le 4}$ contains the renormalizable terms and the $O_i^{(d)}$ are higher-dimensional operators constructed from the light fields. The coefficients $C_i^{(d)}$ are known as Wilson coefficients and encode the effects of the heavy degrees of freedom.
	...
	\chapter{Mapping UV models to EFTs}\label{ch:CDE}
...
%	To analyze a particular physical system embedded in a complex environment, it is essential to isolate the relevant parameters, enabling a simplified description without requiring a complete understanding of the entire system. In situations involving widely separated energy scales, it is often possible to study the low-energy dynamics independently of the high-energy details. The key idea is to identify parameters that are very large or very small compared to the characteristic energy scale of interest and to take appropriate limits. This yields a well-defined approximation that can be systematically improved by including corrections arising from the neglected scales, treated as perturbations.
	...
	\end{align}
	\noindent where $\mathcal{L}(\phi)$ is the Lagrangian density of the theory and $\phi^r(x)$ are the fields. These fields are not necessarily scalars, they might be Dirac or gauge fields.
	...
	\end{align}
	\noindent where the background field is defined to satisfy the classical equation of motion in the presence of the external source: 
	\begin{align}
	...
	This object is equivalent, up to one-loop order, to the $1PI$ generating functional defined by Eq. (\ref{eq:1PIGF}). To see this, consider the relationship between the vacuum expectation value of $\phi^r_c$ the background field $\phi^r_b$, obtained by functional differentiating Eq. (\ref{eq:WJ}) with respect to the source:
	\begin{align}
	...
	%	\begin{tikzpicture}[baseline=-0.09cm]
		...
		To address these issues, the matching condition is instead imposed on the one-light-particle-irreducible (1LPI) generating functional in the UV theory ($\Gamma_{L,UV}[\phi_b]$) and the 1PI generating functional of the EFT\cite{Zhang_2017}:
		\begin{align}
		...
		\noindent where this is imposed for a renormalization scale $\mu \sim M$.
		...
		\end{align}
		\noindent where we already set $J_\Phi = 0$ due our interest in energy scales smaller than the heavy field's mass.	
		...
		Diagrammatically, the background fields correspond to tree lines in Feynman diagrams, while internal lines within loops arise from quantum fluctuations. As a result, terms higher than quadratic in the quantum fields generate interaction vertices that contribute only at higher-loop orders. Since we aim to perform the matching up to a specific order in the loop expansion, we can isolate the relevant contributions to $Z_{UV}[J_\Phi = 0, J_\phi]$ by counting the powers of $\phi'$, $\Phi'$ in the argument. 
		...
		\begin{align}
			...
			\end{pmatrix} = 
			\begin{pmatrix}
				-\frac{\delta^2 \mathcal{L}_{UV}}{\delta\Phi'^2}[\Phi_b,\phi_b] &-\frac{\delta^2 \mathcal{L}_{UV}}{\delta\Phi'\delta\phi'}[\Phi_b,\phi_b]\\
				-\frac{\delta^2 \mathcal{L}_{UV}}{\delta\phi'\delta\Phi'}[\Phi_b,\phi_b] &-\frac{\delta^2 \mathcal{L}_{UV}}{\delta\phi'^2}[\Phi_b,\phi_b]
		...
		\noindent where $\mathcal{L}^{\text{tree}}_{EFT}[\phi]$ and $\mathcal{L}^{\text{1-loop}}_{EFT}[\phi]$ contain effective operators generated at tree and one-loop level, respectively. Furthermore, we also split the fields $\phi$ into background and quantum fluctuations, Taylor expand the Lagrangian plus source around $\phi = \phi_b$, truncate the series at quadratic order in the quantum fluctuations, and substitute in the generating functional (Eq. (\ref{eq:Z_EFT})). Up to one-loop order, we obtain:
		\begin{align}
		...
		To proceed, we must rewrite the quadratic operator of the EFT in term of elements in the first and second term in the equation above. Recall that only the tree-level part of the EFT Lagrangian contributes to $\mathcal{Q}_{EFT}[\phi_b]$ at one-loop. Therefore, substituting Eq.(\ref{eq:L_EFT_tree}) into (\ref{eq:Q_EFT}) and applying the chain rule:
		...
		\begin{align}
			\mathcal{Q}_{EFT}[\phi_b] = -\left[\frac{\delta^2 \mathcal{L}_{UV}[\hat{\Phi}_c[\phi],\phi] }{\delta \phi^2} + \frac{\delta \hat{\Phi}_c[\phi]}{\delta \phi} \frac{\delta^2 \mathcal{L}_{UV}[\hat{\Phi}_c[\phi],\phi]}{\delta \Phi\delta \phi}\right]_{\phi=\phi_b} = \Delta_L + \frac{\delta \hat{\Phi}_c[\phi]}{\delta \phi}\bigg\vert_{\phi = \phi_b} X_{HL}\label{eq:Q_EFT1}
		\end{align}
		...
		\begin{align}
			&\mathcal{Q}_{EFT}[\phi_b] = \Delta_L - X_{LH}\hat{\Delta}_H^{-1} X_{HL}
		\end{align}
		
		Taking the natural logarithm of the determinant:
		\begin{align}
			- i  \ln\det\mathcal{Q}_{EFT}[\phi_b] &= -\frac{i}{2}\ln\det\left(\Delta_L - X_{LH}\hat{\Delta}_H^{-1} X_{HL}\right)\nonumber\\
		...
		\end{align}
		where in the second to third line we used a consequence of the cyclic propertie of the trace~\cite{Zhang_2017}.
		...
		Setting scaleless integrals to zero is a consistent prescription within the expansion by regions method. This is because, when using dimensional regularization, scaleless integrals typically exhibit both UV and infrared (IR) singularities simultaneously, which appear as poles in $1/\epsilon$. These UV and IR poles are known to cancel each other out, leading to a net result of zero for the integral\cite{Jantzen_2011}. This consistency ensures that the sum of contributions from different regions accurately reproduces the expanded form of the original loop integral.
		...
		\begin{align}
			\mathcal{L}^{1-loop}_{EFT}[\phi_b] &= \frac{i}{2}\int \frac{d^dq}{(2\pi)^d}\text{tr}\ln{\left( -P^2 +2q\cdot P - q^2 + M^2 + \left(X_H- X_{HL}\Delta^{-1}_L X_{LH}\right)_{P \to P-q}\right)}_{\text{hard}}\nonumber\\
		...
		This loop integral is given by\cite{PackageX}:
		\begin{align}
		...
		\begin{align}
			&X_H = -\frac{\delta^2 \mathcal{L}_{UV}}{\delta\Phi'^2}\bigg\vert_{\Phi = 0} = \frac{\lambda_1}{2}\phi^2\\
			&X_{LH} = -\frac{\delta^2 \mathcal{L}_{UV}}{\delta\phi'\delta\Phi'}\bigg\vert_{\Phi = 0} = 0\\
			&X_{HL} = -\frac{\delta^2 \mathcal{L}_{UV}}{\delta\Phi'\delta\phi'}\bigg\vert_{\Phi = 0} = 0\\
			&\Delta_L = -\frac{\delta^2 \mathcal{L}_{UV}}{\delta\phi'^2}\bigg\vert_{\Phi = 0} = \left(\partial^2 + m^2 + \frac{\lambda_0}{2}\phi\right)\phi
		\end{align}
		...
		In this simple UV theory without Gauge symmetries, the covariant derivative reduces to the ordinary derivative. Since derivatives act on fields placed to their right, they do not contribute here, simplifying the analysis. Thus, the only surviving term corresponds to a dimension-2 operator:
		\begin{align}
		...
		Computing this loop integral\cite{Zhang_2017}:
		...
		Computing the loop integral\cite{Zhang_2017}:
		\begin{align}
		...
		Recall that $m_L^2$ and $c_4$ have leading order contributions from tree-level matching, along with quantum corrections from one-loop matching, whereas $c_6$ and $c_6'$ arises purely from one-loop corrections. Consequently, the products $m_L^2 c_6'$ and $c_4 c_6'$ consist of two types of terms: one involving a leading-order contribution multiplied by a quantum correction, and another resulting from the product of two loop corrections. Since we are working within a perturbative framework, quantum corrections are expected to be small, and terms involving the product of two such corrections are even smaller. The results ignore 2-loop contributions, so to be consistent these approximations must be made:
		\begin{align}
		...
		Furthermore, since we are truncating the EFT expansion at order $1/M^2$ and both $c_6$ and $c_6'$ already are of order $1/M^2$:
		\begin{align}
		 c_6 + \frac{m_L^2}{2} \frac{c_6'^2}{6!} - \frac{c_4c_6'}{6} = c_6  - \frac{c_4c_6'}{6} + \mathcal{O}(1/M^4)
		\end{align}  
		
		
		Using these relations and redefining the coefficients, we can rewrite the EFT Lagrangian up to one-loop order and up to dimension six as:
		\begin{align}
		...
		Finally, for convenience, we introduce:
		\begin{align}
		...
		%	&\Delta_\Psi = \begin{pmatrix}
			...
			%	&\Delta_{\phi} = \begin{pmatrix}
				...
				\end{align}
				\noindent where it is understood that the product of these matrix contains an implicit integral:
				\begin{align}
				...
				\noindent where the left arrow indicates where the operator is acting. With these definitions of matrix-valued Green's functions, the light fermionic part of the second variation can be expressed as:
				\begin{align}
				...
				Now we can substitute this into Eq. (\ref{eq:variation-part1}), which will lead us to several terms. To keep the expression compact, we define:
				\begin{align}
				...
				\section{Top-philic Scalar Dark Matter}\label{sc:ToyModel}
				...
				In the previous section, we extended the matching procedure used to derive an effective theory to also include the integration of heavy fermions and complex scalar fields from the UV model. To illustrate this method, and especially to explore the CDE of the scalar path integral’s contribution to the one-loop effective Lagrangian, we now apply it to a simplified model based on an extension of the SM. To further simplify the results we consider a simplified version of the SM where there are no leptons, light Yukawas are set to zero and the Higgs field is trated as a real scalar singlet with a mass of $125$ GeV, corresponding to the observed resonance peak. This model introduces a vector-like fermionic top-quark partner ($\psi_T$) and a scalar singlet under the SM Gauge group ($s$).
				...
				\end{align}
				\noindent Here $G^{\mu\nu}$ denotes the field strength tensor associated with the $SU(3)_c$ Gauge group, $q$ represents the first and second generation quarks (light quarks), $t$ is te top quark field, $t_R$ is his right handed component. Furthermore, $T$ is assumed to be a singlet under $SU(2)_L$.
				...
				\end{align}
				\noindent where $\hat{\Psi}_c[\psi,\phi]$ and $\hat{\Phi}_c[\psi,\phi]$ are the classical solutions to the equations of motion for the heavy fields. Therefore, to proceed we solve these classical equations of motion (EOMs), which are derived from the Euler-Lagrangian equations of the UV Lagrangian $\mathcal{L}_{UV}$. There are:
				\begin{align}
				...
				\begin{align}
					...
					- \tilde{\mathbf{X}}_{\Phi\Psi}{\Delta}^{-1}_\Psi\tilde{\mathbf{X}}_{\Psi\Phi} \right) \right]\bigg\vert^{P\to P-q}_{\text{hard}}
				\end{align}
				This structure matches the form obtained in the real scalar case before CDE, allowing us to proceed with an analogous expansion. Following the same steps as in Section~\ref{subsec:CDE_Scalar}, we expand the logarithm to obtain:
				%\begin{align}
				...
				\end{align}
				\noindent where we used the fact that the covariant derivative matrix is proportional to the identity and:
				\begin{align}
				...
				\end{align}
				\noindent where the $P_\Phi$ are ordinary derivatives and, as a consequence, these terms does not contribute since they act on the identity on the right. Furthermore, we can see that all terms will give us operators up to dimension six.
				...
				\noindent where the calculation of this terms can be found in detail in Appendix~\ref{ap:n_1_SF}. To obtain the $n=1$ contribution, we substitute (\ref{eq:l=0}), (\ref{eq:l=1}), (\ref{eq:l=2}), (\ref{eq:l=3}) into Eq. (\ref{eq:n=1}):
				\begin{align}
				...
				We can express the loop integrals that appears using the following notation:
				\begin{align}
				...
				Here, the bolded $\bm{\tilde{\lf}}$ indicates that we have extracted the divergent term of the loop functions. These loop functions were evaluated using Package X \cite{PackageX}. Incorporating these evaluated loop functions, the Lagrangian simplifies to:
				\begin{align}
				...
				\end{align}
				Although it may appear that all terms vanish due to the presence of covariant derivatives at the rightmost position in the expression, this is not the case. By appropriately rearranging the terms, one can express all covariant derivatives within commutators, revealing that these contributions do not vanish. These non-zero terms, in turn, generate pure gauge contributions to the one-loop effective Lagrangian.
				...
				Using the substitution $q^\mu q^\nu \to \frac{g^{\mu\nu}}{d}q^2$ in the loop integral and that $\gamma^\mu\gamma^\nu\gamma_\mu = -(d-2) \gamma^\nu$:
				\begin{align}
					...
					\mathcal{L}^{\text{1-loop}}_{EFT,F}&\supset\frac{i}{2} \int \frac{d^4 q}{(2\pi)^4}   \tr\left[ \frac{ -\frac{1}{2}\gamma^\mu \gamma^\nu P_\mu P_{\nu}q^2 +M_T^2\slashed{P}\slashed{P}  }{ (q^2 - M_T^2)^2 } \right]
				\end{align}
				
				The trace over spinor indices is $\tr(\gamma^\mu\gamma^\nu) = d g^{\mu\nu}$. Therefore:
				\begin{align}
					\mathcal{L}^{\text{1-loop}}_{EFT,F}
					&\supset\frac{i}{2} \int \frac{d^4 q}{(2\pi)^4}   \tr\left[ \frac{ -2 P^2q^2 +4M_T^2 P^2  }{ (q^2 - M_T^2)^2 } \right]
				\end{align}
				
				Since the commutator $[P^\mu,P_\mu] = 0$, there will be no contribution for gauge operators coming from $n=2$. Our next step is to consider the $n=3$ contribution from Eq.~(\ref{eq:exp3}), which is given by:
				\begin{align}
				...
				Using Package X \cite{PackageX}, the loop functions are evaluated as follows:
				\begin{align}
				...
				Using Package X \cite{PackageX} to compute the loop integrals:
				\begin{align}
				...
				\begin{align}
					...
					+\frac{18}{5}\tr(P^\mu P^\nu P^\rho P_\nu P_\mu P_\rho) + ...\Bigg\}
				\end{align}
				These are the only relevant combinations for the matching that we will perform with operators involving only commutators.
				...
				\end{align}
				\noindent where $d^{abc}$ is the totally symmetric third rank tensor and $f^{abc}$ is the $SU(3)$ structure constants.
	...
	All loop integrals and Dirac trace computations are available at \href{https://github.com/viniciusrocca/EFT_ToyModel}{GitHub}. Additionally, this result was cross-checked using \texttt{Matchete}~\cite{Matchete}, and the comparison file is also provided in the same repository.
				
\chapter{Analyses of the EFT Description in $t\bar{t}$ production at LHC}

In the previous chapter, we introduced the Covariant Derivative Expansion (CDE) method and applied it to two models, including a UV BSM model that extends a simplified version of the Standard Model and features a dark matter candidate. Our primary objective there was pedagogical: to develop a detailed understanding of the method and to illustrate applications not extensively treated in the literature.

In this chapter, we shift our focus to a more phenomenologically driven application of EFT. Specifically, we study the process of top-antitop ($t\bar{t}$) production at LHC as a probe to validate the EFT approximation in a realistic extension of the Standard Model. We begin by constructing the EFT Lagrangian for a dark matter (DM) model, now embedded into the full SM rather than a simplified version. We then compare predictions for $t\bar{t}$ production from the EFT against results obtained from the complete one-loop calculation in the UV theory. This comparison is performed both in the region where the EFT is expected to be valid, and in the region where the EFT begins to break down. This will allow us to assess the accuracy and limitations of the EFT description beyond its conventional regime of applicability.

\section{The model}

As in the previous chapter, we introduce two new BSM fields: a vector-like top partner $\psi_T$, which transforms as a color triplet under $SU(3)_c$, and a real scalar singlet $s$, which is neutral under the Standard Model gauge group. The scalar $s$ act as a dark matter candidate, stabilized by an imposed $\mathcal{Z}_2$ symmetry under which both $s$ and $\psi_T$ are odd, while all Standard Model fields are even. The assumed mass hierarchy $M_T > M_s$ also ensures the kinematic stability of $s$ by forbidding its decay into SM particles or into $\psi_T$. The interactions between these BSM fields and the SM are described by the following Lagrangian:
\begin{align}
...
The EFT approach, as discussed in Sec.~\ref{sec:EFT_Limit} relies on the assumption that the relevant energy scales are much smaller than the BSM particle masses. However, at the LHC, it is possible to probe kinematic distributions at energies comparable to BSM mass scales of a few TeV \cite{Lessa_2024}. In such energy regimes, the strict validity of the EFT approximation is not guaranteed, which poses a challenge when utilizing these measurements to search for new physics.
...
To perform the calculations, we implemented the Lagrangians defined in Eqs.~(\ref{eq:L_UV_ttbar}) and (\ref{eq:L_EFT_ttbar}) using FeynArts \cite{FeynArts}, FeynRules \cite{FeynRules} and NLOCT \cite{NLOCT}, exporting the models to the UFO \cite{UFO} format. This allow us to use the Monte Carlo generator \texttt{MadGraph5\_aMC@NLO}~\cite{MG5,MG5NLO} to generate events based on either the full UV one-loop calculation or the EFT approximation. 
...
\end{align}
where the pure BSM squared amplitude, $|\mathcal{M}_{BSM}|^2$, is neglected as it is of higher order in $y_{DM}$ and subleading. In the EFT, this corresponds to retaining only the terms up to $\mathcal{O}(1/\Lambda^2)$, such that the amplitude squared reads:
\begin{align}
	\abs{\mathcal{M}_{EFT}}^2 = \abs{\mathcal{M}_{SM}}^2 + 2 Re(\mathcal{M}_{SM}^*\mathcal{M}_{EFT,\, \text{dim-6}}) 
\end{align}
where $\mathcal{M}_{EFT,\, \text{dim-6}}$ denotes the contribution from dimension-six effective operators. In both cases, the Born-level term $|\mathcal{M}_{SM}|^2$ is identical, as it arises exclusively from QCD interactions in the SM.
...
To carry out these computations, we used the Fixed Order mode of \texttt{MadGraph5\_aMC@NLO} to perform next-to-leading-order (NLO) calculations of the UV one-loop model, aiming for a precision of 10\%. The EFT predictions were obtained at leading order (LO), also using \texttt{MadGraph5\_aMC@NLO}, with a target accuracy of 1\%. All events were generated assuming a center-of-mass energy of \(\sqrt{s} = 13\,\text{TeV}\). We focused on the interference contributions to the total cross section for the subprocesses \(qq \to t\bar{t}\) and \(gg \to t\bar{t}\), isolating the terms proportional to \(g_s^4 y_{DM}^2\). The parameter scan covered the range \((M_T, M_s) = (200, 190)\,\text{GeV}\) to \((10.0, 9.5)\,\text{TeV}\), thereby probing both the regime wh
...
An interesting detail is observed in the lower panel of the $qq\to t\bar{t}$ plot (Fig.~\ref{fig:cs_qq2ttbar_gs4_ydm2}) at $M_T = 1\, \text{TeV}$ and $M_T = 200\, \text{GeV}$. At these specific points, the ratio of the EFT to UV total cross sections seems to deviate from the general behavior observed in the rest of the parameter scan. We are currently investigating the physical reasons behind this anomalous behavior.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.48\linewidth]{qq2ttbar_CS.png}
	\includegraphics[width=0.48\linewidth]{gg2ttbar_CS.png}
	\caption{Graphs showing the interference contribution to the total cross section, proportional to $\alpha_s^2 y_{DM}^2$. The upper panels display the values obtained from the UV one-loop calculation, while the lower panels show the ratio $\sigma_{\text{EFT}} / \sigma_{\text{UV}}$. The left plot corresponds to the $q\bar{q} \to t\bar{t}$ channel, and the right plot to $gg \to t\bar{t}$. In both cases, the BSM mass ratio was fixed at $M_s / M_T = 0.95$, and the error bars reflect the Monte Carlo statistical uncertainty.}

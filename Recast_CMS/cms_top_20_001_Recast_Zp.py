...
def getMTThistAndInfo(file, weightMultiplier = 1.0):
    ...
    # Extracting the new Z' and coupling parameters
    mZp      = float(clean_item(data['mZp']))
    width_Zp = float(clean_item(data['width_Zp']))
    gvd11    = float(clean_item(data['gvd11']))
    gvu11    = float(clean_item(data['gvu11']))
    gvd22    = float(clean_item(data['gvd22']))
    gvu22    = float(clean_item(data['gvu22']))
    gvd33    = float(clean_item(data['gvd33']))
    gvu33    = float(clean_item(data['gvu33']))
    ...
    fileInfo = {
        ...
        'gvd33': gvd33,
        'gvu33': gvu33,
        'xsec (pb)' : float(clean_item(data['xsec (pb)'])), 
        'MC Events' : float(clean_item(data['nevents'])), 
        'file' : file
    }
...
def getRecastData(inputFiles,weightMultiplier=1.0,skipParameters=[]):
    ...
    if not skipParameters:
        ...
        for f in inputFiles:
            ...
            parInfo = (fileInfo['gvd11'],fileInfo['gvu33'],fileInfo['mZp'], 
                        fileInfo['mZp'], fileInfo['model'], fileInfo['process'])
    ...
    if os.path.isfile(outputFile):
        if args.overwrite:
            ...
            for irow,row in df_orig.iterrows():
                skipParameters.append((row['gvd11'],row['gvu33'],row['mZp'], row['mZp'], row['model'], row['process']))
